...
#   filter(word != "") %>%
#   count(report=format(as.character(rdate), format = "%b%Y"),word,sort=TRUE) %>%
...
# print(g2)
# fed_text_by_year <- fed_text %>%
#   mutate(word = gsub("[^A-Za-z ]","",word)) %>%  # keep only letters (drop numbers and special symbols)
#   filter(word != "") %>%
#   count(report=format(as.character(rdate),format="%Y"),word,sort=TRUE) %>%
#   bind_tf_idf(word, report, n) %>%
#   arrange(desc(tf_idf)) %>%
#   anti_join(custom_stop_words3, by="word") %>%
#   mutate(word = factor(word, levels = rev(unique(word)))) %>% 
